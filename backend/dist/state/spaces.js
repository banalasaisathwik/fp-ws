...
async function getSpace(spaceName) {
    await schemaReady;
    const existsResult = await pool.query("SELECT name FROM spaces WHERE name = $1", [spaceName]);
...
async function addClientToSpace(spaceName, ws) {
    const space = await getSpace(spaceName);
    if (space) {
        space.clients.add(ws);
    }
}
async function removeClientFromSpace(spaceName, ws) {
    const space = await getSpace(spaceName);
    if (space) {
        space.clients.delete(ws);
    }
}
async function removeClientFromAllSpaces(ws) {
...
async function addMessageToSpace(spaceName, message) {
    await schemaReady;
    const client = await pool.connect();
    try {
        await client.query("BEGIN");
        const spaceExistsResult = await client.query("SELECT name FROM spaces WHERE name = $1", [spaceName]);
...
async function getMessagesForSpace(spaceName) {
    await schemaReady;
